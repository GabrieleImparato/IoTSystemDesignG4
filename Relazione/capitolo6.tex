...
Per un singolo pacchetto LoRaWAN con 20~byte di \textit{payload}, SF9, BW~=~125~kHz e CR~=~4/5, il tempo di trasmissione (\textit{Time-on-Air}) è circa 165~ms\footnote{Il valore è calcolabile con un \textit{tool} online Semtech o con le formule della specifica SX1276; per semplicità si riporta direttamente il risultato.}. Ogni nodo trasmette una volta ogni 15~minuti, quindi 4 volte all'ora. Il duty-cycle di un singolo nodo è:

\[
    DC_{\text{nodo}} = \frac{4 \times 0{,}165 \text{\,s}}{3600 \text{\,s}} \approx 0{,}018\,\%
\]

\noindent Per verificare la conformità complessiva, si moltiplica per i 17 nodi Classe~A (12 ambientali + 4 suolo + 1 meteo):

\[
    DC_{\text{aggregato}} = 17 \times 0{,}018\,\% \approx 0{,}31\,\%
\]
...
Per stimare l'autonomia di un nodo a batteria si calcola il consumo medio di corrente, tenendo conto che il nodo trascorre la quasi totalità del tempo in stato di riposo (\textit{deep sleep}). La Tabella~\ref{tab:energy_profile} riporta i tre stati operativi di un nodo sensore Classe~A.
...
La Tabella~\ref{tab:stima_costi} riporta una stima di massima del costo dei componenti principali del sistema.
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|X|c|c|c|}
        \hline
        \textbf{Componente} & \textbf{Quantità} & \textbf{Costo unitario (\texteuro)} & \textbf{Costo totale (\texteuro)} \\
        \hline
        Nodo ambientale (MCU + sensori + LoRa + involucro + batteria) & 12 & $\sim$45 & $\sim$540 \\
        \hline
        Nodo umidità suolo (MCU + sonde + LoRa + involucro + batteria) & 4 & $\sim$60 & $\sim$240 \\
        \hline
        Stazione meteo (sensori meteo + MCU + LoRa + alimentazione) & 1 & $\sim$150 & $\sim$150 \\
        \hline
        Nodo attuatore (MCU + LoRa + relè + involucro) & 4 & $\sim$40 & $\sim$160 \\
        \hline
        Gateway LoRaWAN & 2 & $\sim$250 & $\sim$500 \\
        \hline
        Fog node (single-board computer + alimentatore + custodia) & 1 & $\sim$90 & $\sim$90 \\
        \hline
        Router 4G & 1 & $\sim$60 & $\sim$60 \\
        \hline
        Infrastruttura cloud (hosting annuo) & 1 & $\sim$120/anno & $\sim$120/anno \\
        \hline
        \hline
        \textbf{Totale hardware (una tantum)} & & & $\sim$\textbf{1\,740} \\
        \hline
        \textbf{Costo operativo annuo} (cloud + connettività 4G) & & & $\sim$\textbf{240/anno} \\
        \hline
    \end{tabularx}
    \caption{Stima dei costi del sistema (esclusa installazione e manodopera).}
    \label{tab:stima_costi}
\end{table}

\noindent Il costo complessivo dell'hardware è contenuto ($<$2\,000~\texteuro) e risulta ampiamente giustificato dal valore del raccolto protetto, considerando che una singola infezione fungina non gestita può causare perdite di decine di migliaia di euro.


\section{Conclusioni}

Il caso di studio ha dimostrato come i vincoli e i compromessi progettuali individuati nei capitoli precedenti si traducano in scelte architetturali concrete. L'architettura a tre livelli proposta bilancia autonomia locale e funzionalità cloud, mentre le stime ingegneristiche confermano la fattibilità tecnica ed economica del sistema: il duty-cycle rimane ampiamente entro i limiti regolamentari, l'autonomia energetica supera i requisiti minimi e la latenza di allerta è compatibile con gli scenari critici. Il costo contenuto ($<$2\,000~\texteuro{} di hardware) rende la soluzione accessibile anche per un'azienda viticola di medie dimensioni.

Il principale limite del caso presentato è l'assenza di validazione sperimentale: le stime si basano su specifiche nominali dei componenti e su modelli di propagazione teorici. Un eventuale \textit{deployment} reale richiederebbe una fase di \textit{site survey} per verificare la copertura radio, la calibrazione dei sensori e l'ottimizzazione dei parametri di trasmissione alle condizioni specifiche del sito.
