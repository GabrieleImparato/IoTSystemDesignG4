...
Per estendere ulteriormente l'autonomia operativa, specialmente in contesti di agricoltura intensiva come il sito pilota di MATOPIBA, il progetto prevede la sperimentazione di soluzioni per sensori wireless a bassissimo consumo energetico (\textit{ultra-low-power}) basate sull'integrazione di \textit{wake-up radio} combinate con moduli LoRaWAN. % [cite: 355] 
...
Questi ultimi, a loro volta, inoltrano i dati verso i livelli \textit{fog} e \textit{cloud} tramite connessioni IP, come Wi-Fi o reti cellulari (3G/4G). % [cite: 446, 448]
