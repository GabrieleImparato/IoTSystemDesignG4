...
\begin{itemize}
	...
	\item \textbf{Allerta gelate primaverili}: nelle zone collinari irpine, gelate tardive tra marzo e aprile possono danneggiare gravemente i germogli in fase di sviluppo. Un sistema di allarme basato sul rilevamento in tempo quasi reale di temperature prossime a 0\,°C consente di attivare contromisure tempestive (ad esempio ventilatori antigelo\footnote{I ventilatori antigelo (\textit{wind machines}) sono torri alte circa 10~metri utilizzate in pieno campo per contrastare le gelate radiative. Durante le notti serene e calme si forma un'\textit{inversione termica}: l'aria fredda, più densa, ristagna al livello del suolo, mentre a 10--15~m di quota la temperatura è superiore di 2--4\,°C. I ventilatori rimescolano gli strati d'aria, portando quella più calda a contatto con i germogli e innalzando la temperatura quel tanto sufficiente a evitare il danno da gelo.} o bruciatori).
...
\paragraph{Energia.} I nodi sensore saranno installati in pieno campo, privi di accesso alla rete elettrica. L'alimentazione sarà interamente a batteria primaria non ricaricabile, con chimica ad alta densità energetica e bassa autoscarica. Il \textit{power budget} dovrà garantire un'autonomia pluriennale adottando un profilo di \textit{deep sleep} interrotto da brevi cicli di misura e trasmissione, con commutazione a modalità \textit{event-driven} in prossimità delle soglie di gelata.
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{|l|X|X|}
		...
		\hline
		Energia vs. campionamento & Nodi a batteria senza rete elettrica; inverni con scarso irraggiamento; monitoraggio necessario tutto l'anno & Batteria primaria LiSOCl\textsubscript{2} ad alta capacità (non ricaricabile); \textit{deep sleep} con \textit{duty cycling} aggressivo; modalità \textit{event-driven} per soglie critiche \\
		\hline
...
\begin{itemize}
	\item \textbf{Nodi ambientali} ($\sim$12--15 unità): distribuiti nei filari, ciascuno equipaggiato con sensori di temperatura e umidità dell'aria, sensore di bagnatura fogliare e modulo radio LoRa. Alimentazione a batteria primaria LiSOCl\textsubscript{2}.
...
\begin{itemize}
	...
	\item \textbf{Ethernet}: connessione locale tra gateway e fog node.
	\item \textbf{Wi-Fi}: connessione del fog node verso Internet per la sincronizzazione con il livello cloud, tramite router cellulare.
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{|l|X|X|X|}
		...
		\hline
		2 -- Fog & Gateway LoRaWAN, fog node & MQTT, Ethernet, Wi-Fi, NTP & Allerta gelate, comando irrigazione, buffering, sincronizzazione \\
		\hline
...
\noindent Il valore è teorico e volutamente sovrastimato: nella pratica l'autoscarica interna della batteria (1--2\,\% annuo) e il degrado chimico limitano la durata effettiva a circa \textbf{8--10~anni}, comunque ben oltre il requisito minimo di 3--5~anni. La scelta di batterie primarie LiSOCl\textsubscript{2}, non ricaricabili ma con bassissima autoscarica, è coerente con un'installazione a minima manutenzione che non richiede circuiti di ricarica né pannelli solari.
